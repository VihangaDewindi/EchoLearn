...
export default function ParentDashboard() {
  ...
  const openActivityLog = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API}/api/parent/my-child`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const d = await res.json();
      setAllActivity(d?.activity || []);
    } catch {}
    setLoadingAll(false);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFD] flex flex-col font-sans text-left">
      ...
      <main className="flex-1 max-w-[1440px] mx-auto w-full p-8 md:p-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
          ...
          <div className="bg-[#1E2B5A] rounded-[32px] p-8 text-white relative overflow-hidden shadow-xl shadow-blue-900/20">
            <div className="absolute top-0 right-0 p-8 opacity-20"><Layout size={80} /></div>
            ...
            <p className="text-[13px] font-medium text-white/70 mt-4 italic">
              {weeklyGoal.completed >= weeklyGoal.total ? "Goal achieved! 🎉" : `${weeklyGoal.total - weeklyGoal.completed} more to hit the target.`}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
              ...
              {subjects.map((sub: any) => (
                <div key={sub.name}>
                  <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-3">
                      <div className="p-2.5 bg-gray-50 rounded-lg"><Layout size={18} className="text-[#33478D]" /></div>
                  ...
                  <div className="h-3 w-full bg-[#F0F2F5] rounded-full overflow-hidden mb-3">
                    <div className={`h-full ${sub.color} rounded-full transition-all duration-700`} style={{ width: `${sub.progress}%` }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-[32px] border border-[#E5E9F0] shadow-sm overflow-hidden flex flex-col">
                ...
                return (
                  <div key={idx} className="flex gap-4 relative">
                    {idx !== Math.min(activity.length, 5) - 1 && (
                      <div className="absolute left-[20px] top-[40px] bottom-[-32px] w-0.5 bg-[#F5F7FB]" />
                    )}
            ...
            <button
              ...
              className="w-full py-6 text-[12px] font-black text-[#8793AC] hover:text-[#33478D] bg-gray-50/50 uppercase tracking-[0.2em] transition-all"
            >
      ...
      {/* Footer — minimal, consistent with teacher/student style */}
      ...
      {activityLogOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[28px] shadow-2xl w-full max-w-[560px] max-h-[80vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-8 py-6 border-b border-[#F5F7FB]">
              <h2 className="text-[20px] font-black text-[#1E273F]">Full Activity Log</h2>
              <button onClick={() => setActivityLogOpen(false)} className="p-2 hover:bg-gray-100 rounded-xl">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6">
              {loadingAll ? (
                <div className="flex items-center justify-center py-12">
                  <div className="w-10 h-10 border-4 border-[#33478D] border-t-transparent rounded-full animate-spin" />
                </div>
              ) : allActivity.length === 0 ? (
                <p className="text-[#8793AC] font-bold text-center py-12">No activity recorded yet.</p>
              ) : allActivity.map((item: any, idx: number) => {
                const cfg = ACTIVITY_ICONS[item.type] || ACTIVITY_ICONS.login;
                const ItemIcon = cfg.icon;
                return (
                  <div key={idx} className="flex gap-4">
                    <div className={`w-10 h-10 ${cfg.bg} ${cfg.iconColor} rounded-xl flex items-center justify-center shrink-0`}>
                      <ItemIcon size={20} strokeWidth={2.5} />
                    </div>
                    <div className="flex-1">
                      <p className="text-[15px] font-black text-[#1E273F]">{item.title}</p>
                      <p className="text-[12px] font-bold text-[#8793AC] mt-0.5">{timeAgo(item.time)}</p>
                      {item.details && <p className="text-[12px] text-[#5E6D8F] mt-1">{item.details}</p>}
                    </div>
                    {item.score != null && (
                      <span className="text-[11px] font-black text-green-700 bg-green-50 px-2 py-1 rounded-lg h-fit">
                        {item.score}/100
                      </span>
                    )}
                  </div>
